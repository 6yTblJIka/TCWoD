...
namespace LuaPlayer
{
    ...
    {
        E->Push(player->GetUInt32Value(PLAYER_GUILDRANK));
        return 1;
    }
    ...
    {
        E->Push(player->GetUInt32Value(PLAYER_NEXT_LEVEL_XP));
        return 1;
    }
    ...
    int SetGuildRank(Eluna* E, Player* player) // TODO: Move to Guild Methods
    {
        ...
        player->SetUInt32Value(PLAYER_GUILDRANK, rank);
        return 0;
    }
    ...
    {
        E->Push(!player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING));
        return 1;
    }
    ...
    int SetFFA(Eluna* E, Player* player)
    {
        ...
        if(apply)
            ...
            player->SetByteFlag(UNIT_FIELD_BYTES_2, 0, UNIT_BYTE2_FLAG_FFA_PVP);
        return 0;
    }
    ...
    int RewardQuest(Eluna* E, Player* player)
    {
        ...
        player->RewardQuest(quest, 0, nullptr);
        return 0;
    }
    ...
    int CompleteQuest(Eluna* E, Player* player)
    {
        ...
        for (uint32 i = 0; i < quest->Objectives.size(); ++i)
        {
            ...
            {
                player->ModifyCurrency(obj.ObjectID, obj.Amount);
                break;
            }
    ...
    {
        uint32 categoryId = E->CHECKVAL<uint32>(2);
        bool update = E->CHECKVAL<bool>(3, true);

        WorldPackets::Spells::CategoryCooldown cooldowns;
        Unit::AuraEffectList const& categoryCooldownAuras = player->GetAuraEffectsByType(SPELL_AURA_MOD_SPELL_CATEGORY_COOLDOWN);
    ...
    int ModifyCurrency(Eluna* E, Player* player)
    {
        ...
        player->ModifyCurrency(currency, amt);
        return 1;
    }
    ...
    {
        uint32 _icon = E->CHECKVAL<uint32>(2); //2
    ...
    int GossipSendPOI(Eluna* E, Player* player)
    {
        ...
        gossipPoi.Pos = G3D::Vector2(x, y);

        player->SendDirectMessage(gossipPoi.Write());
        return 0;
    }
    ...
    int GossipAddQuests(Eluna* E, Player* player)
    {
        ...
        if (source->GetTypeId() == TYPEID_UNIT)
        {
            ...
            if (unitSource->GetUInt64Value(UNIT_NPC_FLAGS) & UNIT_NPC_FLAG_QUESTGIVER)
