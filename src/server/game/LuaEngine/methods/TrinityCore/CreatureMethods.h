...
namespace LuaCreature
{
    ...
    */
    ...
    int AddThreat(Eluna* E, Creature* creature)
    {
        ...
        creature->getThreatManager().addThreat(victim, threat, SPELL_SCHOOL_MASK_NORMAL, spell ? sSpellMgr->GetSpellInfo(spell) : nullptr);
        return 0;
    }
    ...
    int ClearThreat(Eluna* E, Creature* creature)
    {
        ...
        if (target)
        {
            ThreatContainer::StorageType threatList = creature->getThreatManager().getThreatList();
            for (ThreatContainer::StorageType::const_iterator itr = threatList.begin(); itr != threatList.end(); ++itr)
                if ((*itr)->getUnitGuid() == target->GetGUID())
    ...
    {
        E->Push(creature->GetCreatureTemplate()->npcflag);
        return 1;
    }
    ...
    int SetNPCFlags(Eluna* E, Creature* creature)
    {
        ...
        creature->SetFlag(UNIT_NPC_FLAGS, (NPCFlags)flags);
        return 0;
    }
