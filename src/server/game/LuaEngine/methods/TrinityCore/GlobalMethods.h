...
namespace LuaGlobalFunctions
{
    ...
    int GetAreaName(Eluna* E)
    {
        ...
        E->Push(areaEntry->AreaName_lang);
        return 1;
    }
    ...
    int PerformIngameSpawn(Eluna* E)
    {
        ...
        if (spawntype == 1) // spawn creature
        {
            if (save)
            {
                ...
                if (!creature->Create(map->GenerateLowGuid<HighGuid::Creature>(), map, 0, entry, x, y, z, o))
                {
                    delete creature;
                    E->Push();
                    return 1;
        ...
        if (spawntype == 2) // Spawn object
        {
            ...
            if (save)
            {
                ...
                eObjectMgr->AddGameobjectToGrid(guidLow, ASSERT_NOTNULL(eObjectMgr->GetGOData(guidLow)));
            }
            else
                map->AddToMap(object);
            E->Push(object);
    ...
    int AddVendorItem(Eluna* E)
    {
        ...
        if (!eObjectMgr->IsVendorItemValid(entry, item, maxcount, incrtime, extendedcost, ITEM_VENDOR_TYPE_ITEM))
            return 0;
        eObjectMgr->AddVendorItem(entry, item, maxcount, incrtime, extendedcost, ITEM_VENDOR_TYPE_ITEM);
        return 0;
    }
    ...
    int VendorRemoveAllItems(Eluna* E)
    {
        ...
        for (VendorItemList::const_iterator i = itemlist.begin(); i != itemlist.end(); ++i)
            eObjectMgr->RemoveVendorItem(entry, (*i)->item, 1);
