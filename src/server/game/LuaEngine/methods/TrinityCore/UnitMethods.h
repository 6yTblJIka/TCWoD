...
namespace LuaUnit
{
    ...
    */
    ...
    int GetBaseSpellPower(Eluna* E, Unit* unit)
    {
        ...
        E->Push(unit->ToPlayer()->SpellBaseDamageBonusDone(static_cast<SpellSchoolMask>(spellschool+1)));
        return 1;
    }
    ...
    int SetFFA(Eluna* E, Unit* unit)
    {
        ...
        if (apply)
        {
            ...
            for (Unit::ControlList::iterator itr = unit->m_Controlled.begin(); itr != unit->m_Controlled.end(); ++itr)
                if (Unit* unit = ObjectAccessor::GetUnit(*unit, (*itr)->GetGUID()))
    ...
    */
    ...
    int EmoteState(Eluna* E, Unit* unit)
    {
        ...
        unit->SetUInt32Value(UNIT_NPC_EMOTESTATE, (Emote)emoteId);
        return 0;
    }
    ...
    int DealDamage(Eluna* E, Unit* unit)
    {
        ...
        uint8 difficulty = E->CHECKVAL<uint8>(7);
        SpellInfo* spelnfo = E->CHECKOBJ<SpellInfo>(8);
        ...
        if (!spell)
        {
            DamageInfo dmgInfo = DamageInfo(unit, target, damage, nullptr, schoolmask, SPELL_DIRECT_DAMAGE);
            ...
            unit->SendAttackStateUpdate(HITINFO_AFFECTS_VICTIM, target, 1, SPELL_SCHOOL_MASK_NORMAL, damage, absorb, resist, VICTIMSTATE_HIT, 0);
            return 0;
        }
        ...
        SpellNonMeleeDamage dmgInfo = SpellNonMeleeDamage(unit, target, spell, spellInfo->GetSchoolMask());
