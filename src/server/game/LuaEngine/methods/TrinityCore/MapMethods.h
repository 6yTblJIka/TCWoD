...
namespace LuaMap
{
     ...
     * @param [weatherId] type : the [weatherId], see above available weather WeatherState
    ...
    int SetWeather(Eluna* E, Map* map)
    {
        ...
        uint32 weatherId = E->CHECKVAL<uint32>(3);
        float grade = E->CHECKVAL<float>(4);

        map->SetZoneWeather(zoneId, WeatherState(weatherId), grade);
